...
- Состояние:
  - dev/prod окружения частично реализованы (см. изменения `docker-compose.yml`, `package.json`, `readme.md`);
  - требуется дальнейшее развитие функционала микропроекта (ленивая загрузка компонент) в последующих задачах.
